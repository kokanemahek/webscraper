...
url = "https://www.revisor.mn.gov/statutes/cite/325g.53"
...
if response.status_code == 200:
    ...
    for heading_tag in soup.select('h3'):     
        sibling = heading_tag.find_next_sibling(heading_pattern)
        if heading_tag.find_next_sibling().name == 'p' and sibling:
            if heading_tag.name[1] < sibling.name[1]: 
                new_tag = soup.new_tag('h4',content_type="intro")
                new_tag.string = "[Leading Text]"
                heading_tag.insert_after(new_tag) 

    with open('Ex1/index2.html',"w") as f: 
